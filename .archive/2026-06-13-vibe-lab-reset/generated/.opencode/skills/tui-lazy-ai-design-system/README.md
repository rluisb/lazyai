...
- **Recorded TUI demos (real screen captures):** `demo/01-hero.gif` through `demo/12-*.gif` — copied into `assets/demos/` here for the first six
...
- **Mono (everything terminal):** `JetBrains Mono Nerd Font` — official, ships locally from `fonts/`. Code blocks, command names, file paths, all TUI mockups, all status lines. The Nerd Font variant carries the Unicode glyphs (✓ ⚠ ✗ ⚡ ⠋…) the Bubble Tea TUI relies on.
- **Propo variant** (`JetBrains Mono Propo`) is also wired up for prose where strict fixed-pitch metrics hurt readability.
- **Caveat:** mono is local; Inter is still a Google-Fonts substitute. If the team prefers a different mono (Berkeley Mono, IBM Plex Mono, etc.), swap them and regenerate the type cards.
