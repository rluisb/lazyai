...
Run this inline check after a feature `plan.md` is produced and before the human approval gate. When invoked as the pre-task plan-quality step, read `spec.md`, `plan.md`, and optional `research.md` from research-rpi; do not require or inspect generated task files, and set `checkedAgainst.tasks` to `null`.
...
```json
{
  ...
  "checkedAgainst": {
    ...
    "research": "specs/features/NNN-name/research.md (from research-rpi) or null",
    "tasks": null
  }
}
```
