...
output_schema:
  sections:
    - Clarifications (date, question, answer, decided by)
consumes:
...
- Output is **always** an in-place edit to `specs/{NNN-slug}/spec.md`, in the Clarifications section table.
- Each entry: `| YYYY-MM-DD | <question> | <answer> | <decided by> |`.
...
2. **List** every ambiguity you find. For each, score:
   - **Blast radius**: how many FRs / ACs / phases depend on the answer? (1-5)
   - **Reversibility**: how cheap is it to change later? (1-5)
3. **Sort** by `blast_radius * (6 - reversibility)` descending.
4. **Drop** items whose answer is already implicit in the constitution (cite the article in Clarifications and move on).
5. **Ask** the top item.
...
- Do NOT pick the tech stack, even if asked — redirect to `speckit-plan`.
