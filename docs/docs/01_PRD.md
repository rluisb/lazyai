...
1. When starting a repo, I want to run `lazyai init` so that all supported AI tools receive a consistent baseline setup.
...
`lazyai add` must support:

```text
lazyai add agent reviewer
lazyai add skill pr-review
lazyai add hook protected-paths
lazyai add prompt bugfix-rpi
lazyai add mcp ai-memory
lazyai add standard starter/security
```
...
```text
lazyai validate skills
lazyai validate agents
lazyai validate hooks
lazyai validate mcp
lazyai validate adapters
lazyai validate generated
lazyai validate official-docs
```

### 9.5 Doctor/status/info/list

`lazyai doctor` must inspect:
...
`lazyai info` and `lazyai list` must expose asset catalogs and adapters.

### 9.6 Update and migrate

`lazyai update` must update embedded library assets and adapter definitions with a reviewable diff.
...
- `lazyai validate evals` validates schemas;
...
- `lazyai doctor` must detect inline secrets and warn.
- `lazyai compile` must fail on inline secrets in team profile unless explicitly overridden.
...
```bash
lazyai init --targets opencode,claude,copilot,pi,kiro,antigravity,omp
lazyai validate
lazyai compile --dry-run
...
```bash
lazyai add skill pr-review --pack vibe-lab
lazyai validate skills
lazyai compile --target opencode --target claude --target copilot --target pi
```
...
```bash
lazyai migrate --from existing --targets opencode,claude,copilot
lazyai validate
lazyai compile --dry-run
```
...
```bash
lazyai eject --keep-native --remove-lock
```
...
- Time from `lazyai init` to valid generated setup.
...
- `lazyai init` can create a valid canonical setup.
- `lazyai compile` can emit native outputs for all supported targets.
- `lazyai validate` catches canonical and adapter-specific errors.
- `lazyai doctor` reports host tool compatibility and security warnings.
