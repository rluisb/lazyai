...
Run: lazyai-cli compile --target opencode
-->
```
...
set -euo pipefail
payload="$(cat)"
lazyai-cli hook run protected-paths --payload-json "$payload"
```
...
```bash
lazyai-cli init --targets opencode,claude,copilot,pi,omp,antigravity,kiro
lazyai-cli validate
...
```bash
lazyai-cli init [--targets ...] [--profile team|personal|ci] [--pack vibe-lab/starter] [--yes]
```
...
```bash
lazyai-cli migrate --from existing [--targets ...] [--dry-run]
```
...
```bash
lazyai-cli eject [--keep-native] [--remove-generated-headers] [--remove-ai-dir]
```
...
- `lazyai-cli validate` on a typical repo should complete in under 2 seconds excluding host tool detection.
- `lazyai-cli compile --dry-run` should complete in under 3 seconds for 100 assets.
