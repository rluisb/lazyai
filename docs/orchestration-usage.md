...
- Orchestrator guidance: `.gemini/skills/orchestrator/SKILL.md`

**How to use it**
- Start Gemini from the project root
- Ask it to use the orchestrator skill for a chain/workflow task

**Important limitation**
- Gemini has no native agent/subagent model, so this is best understood as sequential orchestration guidance, not rich multi-agent dispatch

**Example**

```text
Use the orchestrator skill to start the review chain for this change.
```
...
**What ai-setup generates**
- Orchestrator guidance: `.agents/skills/orchestrator/SKILL.md`
- No project-local Codex MCP config file is generated by ai-setup

**How to use it**
- The scaffolded skill gives Codex the orchestrator vocabulary and workflow guidance
- To actually call the orchestration runtime, connect the orchestrator MCP server through your Codex/global MCP configuration first
- Then run Codex from the project root and ask it to use the orchestrator skill

**Important limitation**
- The skill is scaffolded automatically, but MCP connectivity must be handled through Codex’s own MCP configuration path

**Example**

```text
Use the orchestrator skill and start the refactor chain for the cache layer.
```
