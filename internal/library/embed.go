...
// with "claudecode/commands", "opencode/commands", etc.
const GeminiCommandsSubdir = "gemini/commands"

// GeminiCommandsSubdirLegacy is the pre-spec-017 location. Retained as a
// fallback read path for one release so external forks that ship custom
// commands under the old layout keep working. Removal tracked as a
// follow-up to spec 017.
const GeminiCommandsSubdirLegacy = "commands"

// GeminiCommandsDir returns the absolute path to the preferred Gemini
// commands directory (spec 017 layout). Use ResolveGeminiCommandsSubdir
// when reading via an fs.FS so the legacy fallback is honoured.
func GeminiCommandsDir() string { return filepath.Join(Root(), GeminiCommandsSubdir) }

// ResolveGeminiCommandsSubdir returns the library-relative subdirectory to
// read Gemini commands from, using the preferred spec 017 layout when
// present and falling back to the legacy top-level `commands/` directory
// otherwise. Returns empty string if neither exists.
func ResolveGeminiCommandsSubdir(libFS fs.FS) string {
	if libFS == nil {
		return GeminiCommandsSubdir
	}
	if entries, err := fs.ReadDir(libFS, GeminiCommandsSubdir); err == nil && len(entries) > 0 {
		return GeminiCommandsSubdir
	}
	if entries, err := fs.ReadDir(libFS, GeminiCommandsSubdirLegacy); err == nil && len(entries) > 0 {
		return GeminiCommandsSubdirLegacy
	}
	return ""
}
