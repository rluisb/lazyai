...
var ejectCmd = &cobra.Command{
	...
	Long:  "Eject from ai-setup management, converting all managed files to standalone copies that are no longer tracked or updated.",
	RunE:  runEject,
}
...
func runEject(cmd *cobra.Command, args []string) error {
	...
	fmt.Println()
	fmt.Println(headerStyle.Render("🚀 Ejecting from ai-setup"))
	fmt.Println()
	fmt.Printf("  %s This will remove the .ai-setup.json manifest and .ai-setup.db database.\n", warnStyle.Render("⚠"))
	fmt.Printf("  %s Your %d managed files will be kept, but ai-setup will no longer update them.\n", warnStyle.Render("⚠"), numFiles)
	fmt.Println()
