...
import (
	...
	"github.com/ricardoborges-teachable/ai-setup/internal/setupscan"
...
func init() {
	...
	setupCmd.Flags().Bool("adopt", false, "Mark adoptable external configs as ai-setup managed")
	setupCmd.Flags().Bool("import", false, "Import external configs into ~/.ai-setup reference storage")
...
func runSetup(cmd *cobra.Command, args []string) error {
	...
	if actionCount == 0 {
		...
		return fmt.Errorf("no setup action selected (try: ai-setup setup --scan, --list, or --dry-run)")
	}
	if actionCount > 1 {
