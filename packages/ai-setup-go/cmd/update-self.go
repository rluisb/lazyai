...
const latestReleaseURL = "https://api.github.com/repos/ricardoborges-teachable/ai-setup/releases/latest"
...
var updateSelfCmd = &cobra.Command{
	Use:   "update-self",
	Short: "Update ai-setup to the latest GitHub Release",
	Long:  "Download and replace the current ai-setup binary with the latest version from GitHub Releases.",
	RunE:  runUpdateSelf,
}
...
func runUpdateSelf(cmd *cobra.Command, args []string) error {
	...
	fmt.Printf("Downloading ai-setup %s for %s/%s...\n", displayVersion(latestVersion), runtime.GOOS, runtime.GOARCH)

	tempDir, err := os.MkdirTemp("", "ai-setup-update-*")
	if err != nil {
...
func binaryAssetName(goos, goarch string) string {
	name := fmt.Sprintf("ai-setup-%s-%s", goos, goarch)
	if goos == "windows" {
		name += ".exe"
	}
	return name
