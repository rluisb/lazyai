...
describe('wizard integration (non-interactive)', () => {
  ...
  it('global scope scaffolds into ~/.ai and logs unsupported tools', async () => {
    ...
    expect(existsSync(path.join(canonicalDir, 'specs', 'templates', 'plan-template.md'))).toBe(true)
  ...
  it('workspace scope scaffolds planning repo and ledgers without writing into referenced repos', async () => {
    ...
    expect(existsSync(path.join(planningRepoDir, '.specify', 'memory', 'repos', 'fedora', 'ledger.md'))).toBe(true)
    expect(existsSync(path.join(planningRepoDir, '.specify', 'memory', 'repos', 'fedora', 'last-known-state.md'))).toBe(true)
    expect(existsSync(path.join(planningRepoDir, '.specify', 'memory', 'repos', 'creator-checkout', 'ledger.md'))).toBe(true)
