...
func TestCompileMCPForTool_ScopeParity(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			expects: []expect{
				{scope: types.SetupScopeProject, writePathUnder: func(t, _ string) string { return filepath.Join(t, "opencode.json") }},
				{scope: types.SetupScopeWorkspace, writePathUnder: func(t, _ string) string { return filepath.Join(t, "opencode.json") }},
				{scope: types.SetupScopeGlobal, writePathUnder: func(_, h string) string { return filepath.Join(h, ".config", "opencode", "opencode.json") }},
			},
		},
		{
			name: "claude-code",
