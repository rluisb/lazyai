// Package error provides structured error handling for the LazyAI CLI.
...
// AiSetupError is the structured error type for all LazyAI errors.
...
func ManifestNotFound(dir string) *AiSetupError {
	return &AiSetupError{
		Message: fmt.Sprintf("Setup manifest not found in %s. Run 'lazyai-cli init' first.", dir),
...
func ManifestVersion(version string) *AiSetupError {
	return &AiSetupError{
		Message: fmt.Sprintf("Unsupported manifest schema version: %s. Please update lazyai-cli.", version),
