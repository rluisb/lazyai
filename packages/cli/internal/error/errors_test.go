...
func TestManifestUserFacingMessagesUseLazyAICommand(t *testing.T) {
	t.Parallel()

	notFound := ManifestNotFound("/project")
	if !strings.Contains(notFound.Message, "lazyai-cli init") {
		t.Fatalf("ManifestNotFound message = %q, want lazyai-cli init", notFound.Message)
	}
	if strings.Contains(notFound.Message, "ai-setup init") {
		t.Fatalf("ManifestNotFound message still references ai-setup command: %q", notFound.Message)
	}

	version := ManifestVersion("99")
	if !strings.Contains(version.Message, "update lazyai-cli") {
		t.Fatalf("ManifestVersion message = %q, want update lazyai-cli", version.Message)
	}
	if strings.Contains(version.Message, "update ai-setup") {
		t.Fatalf("ManifestVersion message still references ai-setup command: %q", version.Message)
	}
}
