...
func TestScaffoldMcp_PreparesManagedOrchestratorServerFromLocalBuild(t *testing.T) {
	...
	orchestratorDir := filepath.Join(repoRoot, "packages", "orchestrator")
  ...
  "servers": {
	...
	if !filepath.IsAbs(entry.Command) || filepath.Base(entry.Command) != "lazyai-orchestrator" {
		t.Fatalf("command = %q, want prepared absolute lazyai-orchestrator binary", entry.Command)
	}
...
func TestScaffoldMcp_DownloadsOrchestratorReleaseWhenLocalSourceAndPathMissing(t *testing.T) {
  ...
  "servers": {
	...
	if !strings.Contains(entry.Command, filepath.Join("lazyai", "orchestrator", "releases", "v9.9.9", assetName)) {
...
func TestScaffoldMcp_PathOrchestratorPreventsReleaseDownload(t *testing.T) {
	targetDir := t.TempDir()
	libraryDir := t.TempDir()
	mustWriteTestFile(t, filepath.Join(libraryDir, "mcp", "catalog.json"), `{"servers":{"orchestrator":{"command":"lazyai-orchestrator","args":["connect"]}}}`)
	...
	orchestratorLookPath = func(file string) (string, error) {
		if file == "lazyai-orchestrator" {
			return "/usr/local/bin/lazyai-orchestrator", nil
		}
		return "", os.ErrNotExist
	}
	...
	entry := readScaffoldedMcpCatalog(t, filepath.Join(targetDir, ".ai", "mcp.json")).Servers["orchestrator"]
	if entry.Command != "/usr/local/bin/lazyai-orchestrator" {
...
func TestOrchestratorReleaseAssetName(t *testing.T) {
	tests := []struct {
		...
		{goos: "darwin", goarch: "arm64", want: "lazyai-orchestrator-darwin-arm64"},
		{goos: "darwin", goarch: "amd64", want: "lazyai-orchestrator-darwin-amd64"},
		{goos: "linux", goarch: "amd64", want: "lazyai-orchestrator-linux-amd64"},
		{goos: "linux", goarch: "arm64", want: "lazyai-orchestrator-linux-arm64"},
		{goos: "windows", goarch: "amd64", want: "lazyai-orchestrator-windows-amd64.exe"},
...
func TestOrchestratorReleaseURLUsesTaggedReleaseForKnownVersion(t *testing.T) {
	tests := []struct {
		...
		{name: "plain semver", version: "1.2.3", want: "https://api.github.com/repos/rluisb/lazyai/releases/tags/v1.2.3"},
		{name: "leading v", version: "v1.2.3", want: "https://api.github.com/repos/rluisb/lazyai/releases/tags/v1.2.3"},
	}
...
func TestScaffoldMcp_BuildsOrchestratorWhenDistMissing(t *testing.T) {
  ...
  "servers": {
	...
	if !strings.Contains(calls[0], "-C "+orchestratorDir+" build -o ") || !strings.Contains(calls[0], " ./cmd/lazyai-orchestrator") {
...
func TestScaffoldMcp_OptionalOrchestratorSmokeTest(t *testing.T) {
	...
	if got := calls[len(calls)-1]; !strings.HasSuffix(got, "lazyai-orchestrator status") {
