...
- [x] **B.4** OMP + Antigravity surfaced as beta: compile output annotates target name `(beta)` (EC-006), doctor prints a "Beta adapters" advisory for configured beta tools. Tests: `TestCompileSurfacesBetaAdapters`, `TestBetaAdaptersAreOmpAndAntigravity`.
