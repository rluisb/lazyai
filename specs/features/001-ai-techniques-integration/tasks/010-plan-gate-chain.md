...
Move the human approval gate off the `plan` step and onto an explicit sequential `plan-gate` step after `plan-quality`, preserving a simple sequential chain and ensuring plan-quality reports are visible before implementation approval. T8 proved sequential gates are supported and runtime/template conditionals are not; T10 must make only the D6 sequential gate change and must not introduce red-team conditional wiring.
...
- D17 enabled/disabled chain selection logic — T12 owns this.
- Runtime conditional semantics, source-chain template conditionals, and generic orchestration template rendering — T8 proved these are absent/unsupported and T10 must not add them.
...
1. Add failing chain-shape tests expecting step order: `research → plan → plan-quality → plan-gate → implement → review → fix/document` for the D6-only/base path.
...
6. Add failing assertions that the T10 source chain contains no `red-team-plan`, no `{{#if ...}}` markers, no `condition`/`optionalByFeature` runtime metadata, and no parallel block.
7. Only after tests fail, edit source `feature.json` with the smallest sequential D6 change.

## Done When

- [ ] Source `feature.json` remains a sequential `steps` array with no parallel block.
- [ ] Source `feature.json` remains the D6-only/base chain for T10: it does not include `red-team-plan` and does not include runtime/template conditional markers.
...
- **Implementor reaches for unsupported conditionals:** mitigated by T8 evidence references and explicit assertions that T10 adds no condition/template markers.
