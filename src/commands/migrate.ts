...
import { importSetup, detectAdapters } from '../migration/index.js';
...
export function createMigrateCommand(): Command {
  const command = new Command('migrate')
    ...
    .action(async (sourcePath: string | undefined, options: MigrateOptions) => {
      await handleMigrate(sourcePath, options);
    });
...
async function handleMigrate(
  ...
  p.intro(pc.blue('ai-setup migrate'));

  // This is a thin wrapper around import
  // Reuse the same logic but with migrate branding
  const { importSetup, detectAdapters } = await import('../migration/index.js');

  // Validate merge strategy
  const validStrategies = ['smart', 'preserve', 'replace', 'append'];
  const strategy = (options.strategy?.toLowerCase() || 'smart');
  ...
  if (adapters.length === 0) {
    ...
    console.log(pc.gray('Run with a specific path: ai-setup migrate /path/to/project'));
    process.exit(1);
  }
  ...
  // Show plan
  if (result.plan) {
    const { formatPlan } = await import('../migration/plan.js');
    console.log('\n' + formatPlan(result.plan));
  }

  // Check if migration can proceed
  if (!result.success || !result.plan?.canProceed) {
    console.log(pc.red('\n❌ Migration blocked'));
    
    if (result.plan?.conflicts.length > 0) {
      console.log(pc.yellow(`Unresolved conflicts: ${result.plan.conflicts.length}`));
      console.log(pc.gray('Run with --strategy preserve or --strategy replace to override'));
    }
